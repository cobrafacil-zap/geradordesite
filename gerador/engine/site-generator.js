...
const SiteGenerator = (()=>{
  ...
  function renderAbout(project,theme,content){
    // Sobre usa um card estilizado com monograma (em vez de repetir a imagem do hero).
    // Mantém o hero visualmente único.
    const initial = esc((content.company.name||content.company.trade||'★').charAt(0).toUpperCase());
    const name = esc(content.company.name||content.company.trade||'');
    const about = content.about||'Somos uma empresa comprometida com a qualidade e a satisfação dos nossos clientes.';
    return `<section class="alt" id="sobre">
      <div class="wrap about-grid">
        <div class="about-img about-img-noimg">
          <div class="about-monogram" style="background:linear-gradient(135deg,${theme.colors.primary},${theme.colors.secondary});">${initial}</div>
  ...
  // ----- SHARED STYLES (CSS) -----
  // When scope is empty, variables go on :root (used for export ZIPs).
  // When scope is '.canvas-frame', variables stay inside the editor preview
  // and do NOT clash with the host app's :root variables.
  function sharedCSS(theme, scope){
...
.about-img{aspect-ratio:1;background:linear-gradient(135deg,var(--primary),var(--accent));border-radius:14px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:88px;font-weight:700;letter-spacing:0;max-width:280px;margin:0 auto;overflow:hidden;position:relative}
.about-img img{width:100%;height:100%;object-fit:cover;border-radius:14px}
.about-img-noimg{background:transparent;display:block;position:relative;max-width:340px;height:340px}
.about-monogram{width:100%;height:100%;border-radius:18px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:120px;font-weight:800;letter-spacing:-.02em;box-shadow:0 30px 60px -20px rgba(15,23,42,.35),0 8px 18px -6px rgba(15,23,42,.18);font-family:Inter,system-ui,sans-serif}
.about-quote{position:absolute;right:-10px;bottom:-10px;width:90px;height:90px;border-radius:50%;background:var(--accent);color:#fff;font-size:120px;line-height:140px;font-weight:700;font-family:Georgia,serif;display:flex;align-items:center;justify-content:center;box-shadow:0 12px 30px -8px rgba(0,0,0,.3)}
